...
from rag.config.settings import ExtractSettings, get_settings
...
class PdfRouter:
    ...
    def __init__(self, settings: ExtractSettings | None = None) -> None:
    ...
    async def parse(self, content: bytes, source_url: str) -> CanonicalDoc:
        ...
        for page_range in ranges:
            blocks.extend(self._parse_range(content, page_range, source_url))
        if not blocks:
    ...
    def _parse_range(
        ...
        if page_range.page_class is PageClass.SCANNED:
            log.warning(
                "scanned range skipped, OCR is stubbed",
                url=source_url,
...
class ExtractService:
    def __init__(self, settings: ExtractSettings | None = None) -> None:
        self._settings = settings if settings is not None else get_settings().extract
        self._parsers = parser_registry(self._settings)
